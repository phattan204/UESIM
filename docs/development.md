...
The `configure` script auto-detects system capabilities and generates `config.auto.mk`:
...
- `--enable-sctp` - Enable SCTP support (default: auto-detect)
- `--disable-sctp` - Disable SCTP (force TCP fallback)
- `--with-sctp=PATH` - Specify SCTP library path
- `--enable-debug` - Enable debug build
- `--disable-strip` - Disable binary stripping
- `--cross-compile=ARCH` - Cross-compile for architecture (arm, aarch64, x86_64)

**Detection Features:**
- OS detection (Linux, macOS, Windows/MinGW)
- SCTP library detection via header check, pkg-config, or compilation test
- GCC version detection for compatible flags
- Architecture-specific flags

**Output:** Generates `config.auto.mk` with:
```makefile
HAVE_SCTP := 1          # SCTP support enabled
SCTP_LIBS := -lsctp     # SCTP libraries
CC := gcc               # Detected compiler
...
config.mk         # Build configuration (includes config.auto.mk)
rules.mk          # Build rules (SCTP detection, compilation rules)
targets.mk        # Build targets
compress.mk       # Compression and stripping
config.auto.mk    # Auto-generated by configure (DO NOT EDIT)
```
...
#### SCTP Support Detection
```makefile
# In rules.mk - SCTP detection from configure
ifeq ($(HAVE_SCTP),1)
...
COMPILE.c = $(CC) $(DEPFLAGS) $(CFLAGS) $(CPPFLAGS) $(SCTP_CFLAGS) -c
```
...
### Building for RHEL 8.5

**Requirements:**
```bash
sudo yum install -y gcc make libsctp-devel pkg-config
```

**Build Steps:**
```bash
./configure                    # Auto-detect capabilities
make                           # Release build
...
**Without configure:**
```bash
make HAVE_SCTP=1               # Enable SCTP
make BUILD_TYPE=debug          # Debug build
make CROSS_COMPILE=yes CROSS_COMPILER=aarch64-linux-gnu-gcc  # ARM64
```
